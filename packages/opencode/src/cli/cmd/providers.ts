  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const selected = credentials.find(
      (item) =>
        ...
        (args.provider !== undefined && database[item.key]?.name?.toLowerCase() === args.provider.toLowerCase()),
    )
    if (!selected && args.provider) return yield* fail(`Unknown configured provider "${args.provider}"`)
    if (!selected) return
    if (selected.auth === "wellknown") yield* Effect.orDie(authWellKnown.remove(selected.key))
    else yield* Effect.orDie(authSvc.remove(selected.key))
